...
export const getBearerToken = async (): Promise<string | null> => {
  try {
    ...
    // Otherwise, try to get the token from Better Auth's session storage
    // Better Auth stores tokens with the prefix "journal-app"
    const sessionKey = "journal-app_session";
    ...
    if (Platform.OS === "web") {
      ...
      if (sessionData) {
        try {
          ...
          console.error("[API] Error parsing session data:", e);
        }
      }
    } else {
 ...
 * Automatically retrieves session from Better Auth and adds to Authorization header
 *
...
export const authenticatedApiCall = async <T = any>(
  ...
  if (!session?.data?.session?.token) {
    console.error("[API] No valid session found. User needs to sign in.");
    throw new Error("Not authenticated. Please sign in.");
  }

  const token = session.data.session.token;
  console.log("[API] Using auth token for request:", endpoint);
